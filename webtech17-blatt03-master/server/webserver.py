...
class App:
    ...
    def __init__(self, **kwargs):
        ...
        kwargs.setdefault('einstiegsroute', '')
        self.einstiegsroute = kwargs['einstiegsroute']
        kwargs.setdefault('servername', '')
        if self.__class__.__name__ == "IndexApp":  # der servername kann nur von einer IndexApp gesetzt werden
...
class Request:
    ...
    def get_etag(self):
        value=""
        try:
            value=self.headers['If-None-Match']
            return value
        except KeyError:
            return False
...
class Response:
    ...
    def generate_eTag(self, body):
        if type(body) is bytes:
            cbody = body.decode('utf-8')
        else:
            cbody=body

        tag = hashlib.sha256(cbody.encode('utf-8', 'replace')).hexdigest()
        return tag
